...
## 主要能力概述
__计算机视觉__ 
	|目标检测 | 全景分割 | zero-shot图像任务 | cross-domain迁移学习 |

__多模态__ 
	|图文、视频检索 Retrieval | 文本图像生成 | 通用多模态方案,如CLIP, ALBEF, BeitV3 |
...
### __医患沟通用牙齿矫正模拟算法__
* 快速精确的牙齿矫正模拟功能, 紧跟最前沿研究,在latent diffusion发布的最初阶段(2022年8月)即完成选型,定制化训练和本地工程化的工作.
* 针对diffusion族生成式网络进行深入研究,指导设计和架构适用于当前任务的新模型结构和约束，使得生成式模型有效性达到商用标准。

### __全颌可视化系统功能,算法侧架构__
* 基于锥形束CT数据(三维数据)的结构可视化算法,即3D点云的实例分割算法落地和研究优化.
* 多传感器数据融合工作,将多源数据进行高精度配准,数据源包括(CBCT,激光扫描,RGB图像,CT).通过该配准工作掌握了非常多口腔解剖学知识,并在超过一般高精度的配准工作进展中发现了业内暂无人解决的基础性问题,协调组内的研究专家与业内专业教授联合研究。

### __算法在产品商业化上的应用实例__
* 配准用户多次不同时间段的数据，分析形态进展和治疗计划的匹配性，尝试解决商业化上的一个重要痛点（隐形矫正的重启率问题）
* 建立复杂的力学仿真方案，结合CBCT和激光扫描数据用于分析治疗过程中的颌骨和牙齿牙根结构之间的关系，用于产品的风险预警和管控。
